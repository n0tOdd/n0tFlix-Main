    ...
    const visit = (n, snapshotIndex) => {
      ...
      if (!n._string) {
        if (Array.isArray(n[0])) {
          ...
          // Element node.
          const builder = [];
          builder.push('<', n[0]); // Never set relative URLs as <iframe src> - they start fetching frames immediately.
          ...
          for (const [attr, value] of Object.entries(n[1] || {})) {
            ...
            builder.push(' ', attrToSet, '="', escapeAttribute(value), '"');
          }
    ...
    const visit = root => {
      ...
      for (const iframe of root.querySelectorAll('iframe')) {
        ...
        if (!src) {
          ...
          url.searchParams.delete('pointY'); // We can be loading iframe from within iframe, reset base to be absolute.
