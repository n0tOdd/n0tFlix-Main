...
namespace Jint
{
    public class Engine
    {
        ...
        {
            return Execute(source, new ParserOptions(source) { Comment = true, Loc = true, Tokens = true, Tolerant = true, AdaptRegexp = true, Range = true, ErrorHandler = ErrorHandler, });
        }

        public Engine Execute(string source, ParserOptions parserOptions)
        {
            var parser = new JavaScriptParser(source, parserOptions);
            return Execute(parser.ParseScript(false));
        }
        ...
        public Engine Execute(Script program)
        {
            ...
            using (new StrictModeScope(_isStrict || program.Strict))
            {
                ...
                if (result.Type == CompletionType.Throw)
                {
                    ...
                    //todo fix error på ikke implementert her
                    //throw ex;
                }
