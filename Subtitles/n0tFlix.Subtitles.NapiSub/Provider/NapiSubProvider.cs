...
using n0tFlix.Subtitles.NapiSub.Core;
using n0tFlix.Subtitles.NapiSub.Helpers;
            ...
            try
            {
                using (var response = await _httpClientFactory.CreateClient().SendAsync(opts).ConfigureAwait(false))
                {
                    using (var reader = new StreamReader(response.Content.ReadAsStream()))
                    {
                        ...
                        if (status != null && status == "success")
                        {
                            _logger.LogInformation("Subtitles found by n0tFlix.Subtitles.NapiSub");
                    ...
                    _logger.LogInformation("No subtitles found by n0tFlix.Subtitles.NapiSub");
                    return new List<RemoteSubtitleInfo>();
                }
            }
            catch (HttpRequestException ex)
